...
interface ExtendedNonTraitee extends Omit<
  NonTraitee,
...
export default function RecuesNonTraitees() {
  ...
  const [editedOrderLocal, setEditedOrderLocal] =
    useState<ExtendedNonTraitee | null>(null);
  ...
  const isOrderValid = useMemo(() => {
    ...
    return editedOrderLocal.plexuspurchaseOrderLines.every(
      (line: ExtendedPurchaseOrderLine) =>
        line.confirmationStatus && line.confirmationStatus.trim() !== "",
    );
  }, [editedOrderLocal]);
              ...
              <TableBody>
                {table.getRowModel().rows.length > 0 ? (
                  table.getRowModel().rows.map((row) => {
                    ...
                    const isHighlighted =
                      highlightId === String((row.original as NonTraitee).id);
                    return (
                      <Fragment key={row.id}>
                        <TableRow
                          hover
                ...
                (() => {
                  ...
                  const showDateColumn = filteredLines.some(
                    (l: ExtendedPurchaseOrderLine) =>
                      l.confirmationStatus === "Liv pevu a date",
                  );
                                ...
                                <TableRow key={line.id || idx}>
                                  ...
                                  <TableCell>
                                    ...
                                    >
                                      ...
                                      <MenuItem value="Liv pevu a date">
                                        Liv pevu a date
                                      </MenuItem>
                                    </TextField>
                                  </TableCell>
              ...
              try {
                ...
                if (editedOrderLocal.plexuspurchaseOrderLines) {
                  for (const line of editedOrderLocal.plexuspurchaseOrderLines) {
                    const originalLine =
                      editOrder?.plexuspurchaseOrderLines?.find(
                    ...
                    if (
                      ...
                      Number(originalLine.quantity)
                    ) {
                      lineUpdateBody.receiveQuantity = Number(
                        line.deliveryQuantity,
                      );
                    }
                    ...
                    if (line.directUnitCost !== originalLine.directUnitCost) {
                      lineUpdateBody.directUnitCost = Number(
                        line.directUnitCost,
                      );
                    ...
                    if (
                      line.OldRemplacementItemNo !==
                      originalLine.OldRemplacementItemNo
                    ) {
                      lineUpdateBody.OldRemplacementItemNo =
                        line.OldRemplacementItemNo || "";
                    }
                    ...
                    await axiosServices.patch(
                      `/api/purchase-orders/lines/${line.id}`,
                      lineUpdateBody,
                    );
                  }
                }
